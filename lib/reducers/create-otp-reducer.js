...
export function getInitialState(userDefinedConfig) {
  ...
  const transitModeSettings = config?.modes?.transitModes.map((transitMode) => {
    ...
    return {
      ...
      default: true,
      iconName: displayedMode?.toLowerCase?.() || displayedMode,
      key: displayedMode?.toLowerCase?.() || displayedMode,
  ...
  return (prevState = initialState, action) => {
    ...
    switch (action.type) {
      ...
      case 'REALTIME_VEHICLE_POSITIONS_RESPONSE': {
        ...
        return update(state, {
          transitIndex: {
            routes: { [vpRouteId]: { vehicles: { $set: vehicles } } }
          }
        })
      }
      ...
      case 'FIND_STOP_TIMES_FOR_STOP_RESPONSE':
        ...
        // Store stop data under both gtfsId and stopId keys to handle cases where
        // the stopId used in the request differs from the gtfsId in the response
        const stopData = action.payload
        const stopsUpdate = {}
        stopsUpdate[stopData.gtfsId] = { $set: stopData }
        // Also store under stopId if it exists and differs from gtfsId
        if (stopData.stopId && stopData.stopId !== stopData.gtfsId) {
          stopsUpdate[stopData.stopId] = { $set: stopData }
        }
        return update(state, {
          transitIndex: {
            stops: stopsUpdate
