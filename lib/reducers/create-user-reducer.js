...
/**
 * The rider's saved places as state holds them — Home/Work from their legacy
 * keys, the custom places from the savedPlaces key, then any config-suggested
 * locations — read fresh from localStorage.
 *
 * Shared by the boot load and RESTORE_LOCAL_USER_PLACES (backlog 28.1), which
 * rebuilds this list after the server copy has written back what the phone had
 * lost: one reader, so a restored place is shaped exactly like a booted one.
 */
function loadSavedLocations(configLocations) {
...
function loadUserFromLocalStorage(config) {
  ...
  const locations = loadSavedLocations(configLocations)
...
function createUserReducer(config) {
  ...
  return (state = initialState, action) => {
    switch (action.type) {
      ...
      case 'RESTORE_LOCAL_USER_PLACES':
        // The places-sync thunk has already written the restored keys
        // (lib/actions/places-sync.ts); state is rebuilt from them so nothing
        // on screen disagrees with what the next launch will read.
        return update(state, {
          localUser: {
            savedLocations: { $set: loadSavedLocations(config.locations) }
          }
        })
