...
export function setLocationToCurrent(payload, intl) {
  return function (dispatch, getState) {
    ...
    // Aboard a bus (sticky Go Mode riding state), the rider's actionable
    // position is the next stop on their line — not the moving mid-street GPS
    // point, which reverse-geocodes to nonsense like "Lyndale Ave S & 90th St".
    // Use the stop directly and skip reverse geocoding: it is already named.
    if (state.otp.goMode?.isActive && state.otp.goMode.riding) {
      const nextStop = getNextStopOnRide(state)
      if (nextStop) {
        dispatch(
          settingLocation({
            ...payload,
    ...
    // Keep the field reading "(Current Location)". Reverse geocoding here used
    // to replace the label with a raw street address AND drop the
    // CURRENT_LOCATION category, so nothing downstream could tell the origin
    // was live GPS (no per-search coord refresh, recents pollution, rider
    // confusion — 07-11 ride note).
    dispatch(
      settingLocation({
        ...payload,
        location
