...
import { builtInTypeForName, toBuiltInPlace } from '../util/saved-places'
import {
  ...
  configModes.modeButtons.forEach(({ modes }) => {
    modes.forEach(({ mode }) => {
      if (isTripTransit && isTransit(mode)) {
        ...
        selectedButtons.add(mode?.toLowerCase?.() || mode)
      }
    })
  })
...
export function rememberPlace(placeTypeLocation, intl) {
  return function (dispatch, getState) {
    ...
    if (persistenceMode.isOtpMiddleware) {
      ...
      const { location, type } = placeTypeLocation
      // A place the rider named "Home"/"Work" fills the built-in slot instead
      // of becoming a custom row beside an empty one (2026-09-04: two "Home"
      // rows, the built-in one still reading "Set your home address").
      // Recents are excluded: their names are geocoder addresses, not
      // nicknames the rider chose.
      const slot =
        type === 'recent' || isHomeOrWork({ type })
          ? null
          : builtInTypeForName(location?.name)
      if (slot) {
        // Drop the custom row being promoted, so editing an existing custom
        // "Home" moves it into the slot rather than leaving both.
        if (location?.id) dispatch(deleteLocalUserSavedPlace(location))
        return dispatch(
          rememberLocalUserPlace({
            location: convertToLegacyLocation(toBuiltInPlace(location, slot)),
            type: slot
          })
        )
      }
...
export function forgetPlace(placeId, intl) {
  return function (dispatch, getState) {
    ...
    // Custom saved-place ids are minted as `place-${randId()}` and must never
    // contain that substring, or they would be deleted down the wrong branch.
