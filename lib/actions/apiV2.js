...
import { getRouteColorBasedOnSettings, routeIsValid } from '../util/viewer'
      ...
      ?.map((leg) => {
        ...
        return {
          ...
          route: {
            ...
            textColor: getRouteColorBasedOnSettings(
              routeOperator,
              routeProperties
            ).split('#')?.[1]
          }
...
export function routingQuery(searchId = null, updateSearchInReducer) {
  ...
  return function (dispatch, getState) {
    ...
    if (!queryIsValid(state)) {
      console.debug('Query is invalid. Aborting routing query', currentQuery)
      ...
      const variables = {
        ...
        ...(capBearing
          ? stopCapVariables(
              ...
              config?.itinerary?.maxStopCount,
              false,
              !!config?.itinerary?.enableMaxStopCount
            )
          : null),
        ...planConstraintVariables(combo)
      }
    ...
    const dispatchPlan = ({ combo, index, query, variables }) =>
      dispatch(
        createGraphQLQueryAction(
          planQuery ||
            extendPlanQueryWithLevers(query.query, {
              enableMaxStopCount: !!config?.itinerary?.enableMaxStopCount
            }),
          variables,
          (response) => {
