...
export function createGraphQLQueryAction(
  ...
  responseAction: (response: unknown) => void,
  errorAction: (error: unknown) => void,
  options: { batchId?: string; [key: string]: unknown } = {}
...
export const findTrip = (params: { tripId: string }): ((dispatch: any, getState: any) => Promise<void>) =>
...
export const findNearbyStops = ({
  ...
  max,
  radius = 1000
}: { focusStopId: string; lat: number; lon: number; max: number; radius?: number }): ((dispatch: any, getState: any) => Promise<void>) => {
...
export const findRoutesNearby = ({ lat, lon, radius = 250 }: { lat: number; lon: number; radius?: number }): ((dispatch: any, getState: any) => Promise<void>) =>
  createGraphQLQueryAction(
    `{
      stopsByRadius(lat: ${lat}, lon: ${lon}, radius: ${radius}) {
        edges {
