...
import { setNotifyLocale } from '../util/go-mode/notify-i18n'
...
export const setMapPickMode = createAction('SET_MAP_PICK_MODE')

/**
 * Rider ask (backlog 3.9): put the map into "pick a point" mode for one end of
 * the trip, so a place with no name — a trailhead, a park entrance, the far
 * side of a lot — can be set by tapping the map instead of typing an address.
 *
 * The mode is redux state rather than component state on purpose: on a phone
 * the location picker is a full screen of its own (mobile/location-search),
 * so the screen that started the pick is gone by the time the rider taps the
 * map. Leaving the full-screen picker is part of starting the pick.
 */
export function startMapPick(locationType) {
  return function (dispatch, getState) {
    const { mobileScreen } = getState().otp.ui
    dispatch(setMapPickMode({ locationType }))
    if (
      mobileScreen === MobileScreens.SET_FROM_LOCATION ||
      mobileScreen === MobileScreens.SET_TO_LOCATION ||
      mobileScreen === MobileScreens.SET_INITIAL_LOCATION
    ) {
      // The map only exists behind the search form screen.
...
/**
 * Leave map-pick mode without setting anything.
 */
export function cancelMapPick() {
  return setMapPickMode({ locationType: null })
}
...
export function setMainPanelContent(payload) {
  return function (dispatch, getState) {
    ...
    if (
      ...
      //
      // null -> null at the root is not an over-enthusiastic route; it is app
      // start-up asking for the panel it is already on, and it happens on every
      // load that restores a query. Warning about it made "Attempt to route
      // from null to null" a fixture of every ride log, where it read as a
      // fault. Say something only when a real transition was refused.
      if (payload != null || otp.ui.mainPanelContent != null) {
        console.warn(
...
export function setViewedStop(payload, viewer = 'nearby') {
  return function (dispatch, getState) {
    ...
    if (payload) {
      ...
      // A null payload means "close the stop viewer", and until now it did
      // nothing at all: every caller that passed null (go-mode's
      // returnToGoMode among them) left ui.viewedStop exactly as it found it.
      dispatch(viewStop(null))
    }
  }
}
...
/**
 * A stop tapped from inside an itinerary (TransitLegSubheader's "View stop").
 *
 * Normally the app closes the main panel and hands the rider the nearby view.
 * Under Go Mode that is unreachable: the tracking screen is a fixed
 * full-screen layer (z-index 1000) and the desktop layout does not render the
 * main panel at all while a trip is running, so the rider taps their next
 * boarding stop and nothing appears to happen. Rider ask #39, 2026-08-27:
 * a "next bus" view on the trip when a connection is missed.
 *
 * So mid-trip, set `ui.viewedStop` directly — the state the app's own
 * StopScheduleViewer reads — and let the Go Mode layer render that viewer over
 * the trip. Deliberately NOT setViewedStop(payload, 'stop'): that also pushes
 * `/schedule/<id>` onto the history, and re-routing the URL out from under a
 * live trip is how a mid-ride tap turns into a re-mounted trip. `inGoMode`
 * marks who asked, so the mobile shell leaves the trip screen up instead of
 * replacing it (a stop reached from the app menu still opens full-screen, as
 * it always has).
 */
export function viewStopFromItinerary(payload) {
  return function (dispatch, getState) {
    const { goMode } = getState().otp
    if (goMode?.isActive && !goMode?.ui?.backgrounded) {
      dispatch(viewStop({ ...payload, inGoMode: true }))
      return
    }
    dispatch(setMainPanelContent(null))
    dispatch(setViewedStop(payload))
  }
}
  ...
  return function (dispatch, getState) {
    ...
    if (previousSearch) {
      ...
      if (activeSearchId && activeSearchId !== previousSearchId) {
        ...
        // Active itinerary index has changed. The key travels with it so the
        // history entry keeps naming the trip the rider chose there, not
        // whatever that position holds now (backlog 23.5).
        dispatch(
          setActiveItinerary({
            index: previousItinIndex,
            key: urlParams.ui_activeItineraryKey
          })
        )
      }
    } else {
...
export function setLocale(locale, isUpdatingLocale = false) {
  return async function (dispatch, getState) {
    ...
    if (matchedLocale !== ui.locale) {
      ...
      // Go Mode's alert copy formats outside React — the notification builders
      // are called from the Go Mode tick, which has no IntlProvider above it —
      // so the same pair is registered for it here (backlog 12.23). Beside the
      // dispatch on purpose: one place changes the locale, one place tells
      // both consumers.
      setNotifyLocale(matchedLocale, messages)
