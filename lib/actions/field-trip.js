...
function makeSaveFieldTripItinerariesData(request, outbound, state) {
  ...
  if (!callTaker) return null
  const { fieldTrip, session } = callTaker
...
function getTripIdsForTravelDate(request) {
  return async function (dispatch, getState) {
    ...
    const callTaker = state.callTaker
  if (!callTaker?.session) return null
  const { sessionId } = callTaker.session
...
function getMissingTripHashesForActiveItineraries() {
  return async function (dispatch, getState) {
    ...
    const callTaker = state.callTaker
  if (!callTaker?.fieldTrip) return null
  const { tripHashLookup } = callTaker.fieldTrip
...
function checkValidityAndCapacity(state, request) {
  ...
  const callTaker = state.callTaker
  if (!callTaker?.fieldTrip) return null
  const { travelDateTripsInUse, tripHashLookup } = callTaker.fieldTrip
