...
// Suppress React warning about missing keys in ModeSelector3 in production
// This warning comes from an external component (@opentripplanner/trip-form)
// that we cannot modify directly. The warning is harmless but noisy.
if (process.env.NODE_ENV === 'production') {
  const originalError = console.error
  console.error = (...args) => {
    // Filter out the specific React warning about missing keys in ModeSelector3
    if (
      args.length >= 1 &&
      typeof args[0] === 'string' &&
      args[0].includes('Warning: Each child in a list should have a unique "key" prop') &&
      args[0].includes('ModeSelector3')
    ) {
      // Skip this specific warning in production
      return
    }
    originalError(...args)
  }
