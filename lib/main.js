...
import { notePlacesWrite, startPlacesSync } from './actions/places-sync'
...
if (isTripActive()) noteGoModeActivity(pendingBundleDeps)
...
store.subscribe(() => {
  ...
  if (goMode?.isActive && goMode.activeItinerary) {
    ...
    clearGoModeSession()
    // A trip ended — which is NOT yet the first safe moment. Applying here is
    // what 15.6 was: on 2026-09-13 this line installed 2026.0913.2 18 ms after
    // endGoMode released the hold, while the rider was standing on a train
    // between two onboard flows. So start the quiet period instead, and hand
    // it the apply to run at the far end of it: no Go Mode for
    // GO_MODE_QUIET_PERIOD_MS, and a bundle held all ride finally lands
    // without the rider ever having to relaunch.
    if (wasActive) {
      beginGoModeQuietPeriod({
        ...pendingBundleDeps,
        onQuiet: () => applyPendingBundleWhenSafe(pendingBundleDeps)
      })
    }
  }
})

// Backlog 28.1: every change to the rider's places — a save, an edit, a
// delete, a recent — is beaconed (PLACES_STATE, counts only) and, for saved
// places, copied to the server. Watched here rather than in the reducer, which
// must stay pure of network; the user slice's arrays are replaced on every
// write (immutability-helper), so a reference check is the whole test.
let lastSavedLocations = store.getState().user?.localUser?.savedLocations
let lastRecentPlaces = store.getState().user?.localUser?.recentPlaces
store.subscribe(() => {
  const localUser = store.getState().user?.localUser
  const savedLocations = localUser?.savedLocations
  const recentPlaces = localUser?.recentPlaces
  if (
    savedLocations === lastSavedLocations &&
    recentPlaces === lastRecentPlaces
  ) {
    return
  }
  const places = savedLocations !== lastSavedLocations
  lastSavedLocations = savedLocations
  lastRecentPlaces = recentPlaces
  store.dispatch(notePlacesWrite({ places }))
})
...
// What the phone holds at this launch (the boot PLACES_STATE beacon, after
// loadUserFromLocalStorage has run inside createStore), then the server copy
// merged back in wherever a key came up missing or empty. Never throws; a
// failed GET leaves local state alone. Backlog 28.1.
store.dispatch(startPlacesSync())
