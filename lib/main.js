...
// What the bundle-apply gate needs to know. Kept next to the store because
// both answers are about THIS app instance: whether a trip is running, and
// where a decision should be recorded.
const pendingBundleDeps = {
  // A saved session counts as active as well as a live one: at boot the
  // resume has not necessarily run yet, and a rider who force-quit mid-ride
  // and reopened the app is mid-ride, not idle.
  isTripActive: () =>
    store.getState().otp?.goMode?.isActive === true ||
    loadGoModeSession() != null,
