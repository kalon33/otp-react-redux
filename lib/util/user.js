...
export function convertToPlace(location) {
  const { icon, id, lat, lon, name, timestamp, type } = location
  return {
    address: name,
    icon: isWork(location) ? 'briefcase' : icon,
    // Preserve id and timestamp so reloaded recent places can still be
    // forgotten (matched by id) and show a real "x days ago".
    id,
    lat,
    lon,
    name,
...
export function convertToLegacyLocation(place) {
  const { address, icon, id, lat, lon, name, timestamp, type } = place
  return {
    ...
    name: isHomeOrWork(place)
      ...
      ? `${name} (${address})`
      : address || name,
    // Keep the timestamp so recent places survive a reload with a real
    // "x days ago" instead of NaN.
    timestamp,
    type
  }
}
