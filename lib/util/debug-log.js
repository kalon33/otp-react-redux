 ...
 *   session isn't lost.
 * - OPT-IN: nothing is captured or sent unless localStorage.otpDebugLog === '1'
 *   (set via the app-menu "Share diagnostics" toggle or ?debugLog=1). New users
 *   default to off; the deployment owner opts their own devices in.
 */
...
function isEnabled() {
  try {
    return (
      ...
      window.localStorage?.getItem('otpDebugLog') === '1'
    )
  } catch {
