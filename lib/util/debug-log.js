...
// Cached once: is trip-recording opted in for this session? (URL ?recordTrip=1
// or localStorage.otpRecordTrip === '1'.) The actual full-capture also requires
// a Go Mode trip to be active — see isRecordingTrip().
let recordFlag = false
function readRecordFlag() {
  try {
    if (typeof window === 'undefined') return false
    if (window.localStorage?.getItem('otpRecordTrip') === '1') return true
    return new URLSearchParams(window.location.search).get('recordTrip') === '1'
  } catch {
    return false
  }
}
...
function summarisePayload(action, full) {
  try {
    ...
    if (full) {
      if (json.length <= MAX_FULL_PAYLOAD_CHARS) return action.payload
