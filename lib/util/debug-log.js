...
//   client entry  <= MAX_FULL_PAYLOAD_CHARS (320k)
//   Flask line    <= DEBUG_LOG_MAX_LINE_CHARS (384k, preferences_api.py)
...
//
// These are a LADDER and must move together, strictly increasing. Raising only
// the client ceiling relocates the loss one hop instead of removing it: the
// Flask cap replaces an over-long line with a truncation stub of its own.
//
// 200k was too low twice on 2026-08-27. Both rides of that day recorded a
// START_GO_MODE of 270,837 chars — a six-leg itinerary with full geometry — and
// both collapsed to the summary stub, so neither trip could be replayed and the
// worst Go Mode bug of the day had to be diagnosed from the action stream by
// hand. 270,837 also exceeded the 256k Flask cap, which is why that moved too.
const MAX_FULL_PAYLOAD_CHARS = 320000
