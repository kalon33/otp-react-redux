...
import {
  ...
  isUrgentEntry,
  shouldUrgentFlush
} from './debug-log-urgency'
import { measureEntry } from './debug-log-entry'
import { PAYLOAD_SUMMARISERS } from './debug-log-summarisers'
...
// Backlog 20.2: a failure-class entry is beaconed the moment it is recorded
// rather than waiting up to FLUSH_INTERVAL_MS, because an iOS force-quit fires
// no `pagehide` and takes the buffer with it — and a rider staring at a failure
// card is the rider about to force-quit. flushBeacon never splices and the sink
// dedupes by entry id, so the worst case is a copy the server drops. Which
// entries qualify, and the rate limit, live in debug-log-urgency.js.
let lastUrgentFlushAt = 0
// Runs of failed flushes, so a stream blocked at the transport is not mistaken
// for an app that was never opened. Same module, same reason.
const flushFailures = createFlushFailureTracker()
...
//     Since 2026-09-15 the quiet access re-plan emits this type too, tagged
//     `request.reason` = quiet-replan-scoped / quiet-replan-full: it uses the
//     same isolated fetch and was equally invisible, which is why the 09-15
//     ride's nine itinerary swaps have no recorded requests behind them
//     (backlog 13.8). build-fixture.js splits the two on that tag.
...
/**
 * Buffer an entry with its identity and size accounting. No sending, and no
 * consent gate — callers that are not push() are the sink's own breadcrumbs,
 * which are minted behind the same gate already.
 */
...
function push(entry) {
  if (!isEnabled()) return
  bufferEntry(entry)
  // Sent only after the entry is buffered and past the eviction caps, so the
  // beacon carries it rather than racing it. flushUrgent lives with the other
  // senders further down (it needs flushBeacon); this is the one forward
  // reference that buys, rather than costs, readability.
  // eslint-disable-next-line @typescript-eslint/no-use-before-define
  if (isUrgentEntry(entry)) flushUrgent()
}
...
function summarisePayload(action, full) {
  try {
    ...
    // Named summariser first: the whole point is to avoid stringifying a
    // payload we already know we are not going to keep. `hasOwnProperty` so an
    // action type called "constructor" can't reach Object.prototype.
    if (
      Object.prototype.hasOwnProperty.call(PAYLOAD_SUMMARISERS, action.type)
    ) {
      return PAYLOAD_SUMMARISERS[action.type](action.payload)
    }
...
/** Record that a flush was rejected, once per run of failures. */
function noteFlushFailed() {
  inFlightSince = 0
  const crumb = flushFailures.fail(Date.now())
  if (crumb) bufferEntry(crumb)
}

/** Record that the link came back, with how long it was out. */
function noteFlushRecovered() {
  const crumb = flushFailures.recover(Date.now())
  if (crumb) bufferEntry(crumb)
}
...
/** Beacon what is buffered now, rate-limited. Never throws into the caller. */
function flushUrgent() {
  try {
    const now = Date.now()
    if (!shouldUrgentFlush(now, lastUrgentFlushAt)) return
    lastUrgentFlushAt = now
    flushBeacon()
  } catch {
    // Diagnostics must never break the path they are diagnosing.
  }
}
