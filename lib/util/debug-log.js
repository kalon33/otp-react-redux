...
const FULL_CAPTURE_TYPES = new Set([
  ...
  'SET_ONBOARD_TRIP',
  'SET_ONBOARD_RESULT',
  // The five onward plans an onboard optimize actually ranks, one per candidate
  // alight stop. SET_ONBOARD_RESULT above is the OUTPUT; this is the input the
  // ranking was computed from, and without it a fixture cannot replay the step
  // between the two. On 2026-08-09 that gap is why the backwards-itinerary
  // proof had to be a unit test rather than a replay gate: these plans resolve
  // through a local promise in fetchOnboardCandidatePlan and never dispatch
  // ROUTING_RESPONSE, so nothing ever recorded them.
