 ...
 * fixture's name. `--since`/`--until` cut the recording down to one ride.
 *
 * Nor is a START_GO_MODE a ride boundary, which is why there is no --nth-trip:
 * `beginGoMode` re-dispatches START_GO_MODE on every itinerary swap, so a quiet
 * access re-plan looks exactly like a fresh trip. That same session carries
 * ELEVEN of them for two rides. An instant is the only unambiguous cut.
 *
 * Usage:
 ...
 *                    ISO 8601 date — '2026-08-29T01:56:44Z')
...
function parseArgs(argv) {
  const args = { latest: false, sinceMs: -Infinity, untilMs: Infinity }
...
module.exports = { dedupeKey, parseArgs, parseTime, readAllEntries }
