...
export async function fetchOnboardContext(lat, lon, vehicleRadiusM) {
  ...
  // feeds=all: the graph carries more than one agency (Metro Transit is feed
  // "1", MVTA "2"), and asking for "all" rather than naming them keeps the
  // feed numbers out of this bundle entirely — a third operator appearing in
  // the graph is then discoverable with no app release. The sidecar answers
  // with ready-made "2:465" ids; omitting the param would get the pinned
  // legacy scope, which is Metro Transit only.
  const [vehicles, routes] = await Promise.all([
    getJson(
      `/api/onboard/vehicles-near?${point}&radius=${Math.round(
        vehicleRadiusM
      )}&feeds=all`
    ),
    getJson(`/api/onboard/routes-at-point?${point}&radius=100&feeds=all`)
  ])
