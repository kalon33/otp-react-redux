 ...
 *
 * Shape: `<sessionId>[.<loadTag>]-<seq base36>`, e.g. `mtbtyif4-axo9cm-1a`.
 *
 ...
 */
export function createEntryIdMinter(sessionId, loadTag) {
  const base =
    typeof sessionId === 'string' && ENTRY_ID_PATTERN.test(sessionId)
      ? sessionId
      : 'anon'
  // A second app start writing under a session id it inherited (a ride the app
  // re-mounted inside — see debug-log.js adoptSessionId) must not re-issue that
  // session's ids from zero: the sink dedupes on the id, so every entry of the
  // second load would look like a re-send of the first's and be dropped. The
  // tag makes the two loads distinct while leaving each one's counter dense, so
  // a gap still means what it has always meant. `.` is inside ENTRY_ID_PATTERN
  // and the server's identical one.
  const tag =
    typeof loadTag === 'string' && loadTag && ENTRY_ID_PATTERN.test(loadTag)
      ? `.${loadTag}`
      : ''
