...
/**
 * Collect the Go Mode re-route itineraries as a browsable list: de-duplicated,
 * sorted shortest-duration first, capped to a handful. Returns [] until results
 * arrive. (getRerouteCandidate keeps returning just the single best for callers
 * that only need one.)
 */
export function getRerouteCandidates(state, limit = 5) {
  const search = getRerouteSearch(state)
  const responses = search?.response || []
  const itineraries = responses
    .filter((res) => res && !res.error && res.plan?.itineraries)
  ...
  // The multi-combination search can surface the same option more than once;
  // collapse by a lightweight leg signature (mode + route + endpoints).
  const seen = new Set()
  const unique = []
