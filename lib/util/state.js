...
import { scoreAlightOption } from './go-mode/alight-optimizer'
...
/**
 * Get the dedicated Go Mode re-route search object (if a re-route is underway).
 */
export function getRerouteSearch(state) {
  const searchId = state.otp.goMode?.reRoute?.searchId
  return searchId ? state.otp.searches[searchId] : null
}

/**
 * Pick the best (shortest-duration) itinerary from the Go Mode re-route search,
 * across all mode combinations. Returns null if none have arrived yet.
 */
export function getRerouteCandidate(state) {
  const search = getRerouteSearch(state)
  const responses = search?.response || []
  const itineraries = responses
    .filter((res) => res && !res.error && res.plan?.itineraries)
    .flatMap((res) => res.plan.itineraries)
  if (itineraries.length === 0) return null
  return itineraries.reduce((best, itin) =>
    itin.duration < best.duration ? itin : best
  )
}

/**
 * True once the re-route search has settled (all combination responses are in).
 */
export function isRerouteSearchSettled(state) {
  const search = getRerouteSearch(state)
  return !!search && !search.pending
}

/**
 * For the "I'm on the bus" flow: across all candidate alight-stop searches that
 * have settled, pick the stop whose total arrival time (bus arrival at the stop
 * + remaining-journey duration) is earliest. Ties break on fewer transfers,
 * then less walking. Returns null until at least one candidate has results.
 */
export function getBestAlightOption(state) {
  const candidateSearches = state.otp.goMode?.onboard?.candidateSearches || []
  if (candidateSearches.length === 0) return null

  let best = null
  candidateSearches.forEach((cs) => {
    const search = state.otp.searches[cs.searchId]
    if (!search || search.pending) return
    const itineraries = (search.response || [])
      .filter((res) => res && !res.error && res.plan?.itineraries)
      .flatMap((res) => res.plan.itineraries)
    if (itineraries.length === 0) return

    const bestForStop = itineraries.reduce((b, itin) =>
      itin.duration < b.duration ? itin : b
    )
    const arrival = scoreAlightOption(cs.busArrivalEpoch, bestForStop)

    // The rider's intent is to STAY on the bus they're already aboard, so when
    // two alight stops arrive within a few minutes of each other treat them as
    // tied and prefer the one with fewer onward transfers (then less walking,
    // then earlier arrival). This keeps the rider on the current vehicle to a
    // convenient stop rather than getting off early to shave a few seconds.
    const TIE_MS = 180000
    let better
    if (!best) {
      better = true
    } else if (Math.abs(arrival - best.arrival) <= TIE_MS) {
      const dT = (bestForStop.transfers ?? 0) - (best.itinerary.transfers ?? 0)
      const dW =
        (bestForStop.walkDistance ?? 0) - (best.itinerary.walkDistance ?? 0)
      better =
        dT < 0 ||
        (dT === 0 && dW < 0) ||
        (dT === 0 && dW === 0 && arrival < best.arrival)
    } else {
    ...
    if (better) {
      best = {
        arrival,
        busArrivalEpoch: cs.busArrivalEpoch,
...
/**
 * True once every candidate alight-stop search has settled.
 */
export function isOnboardSearchSettled(state) {
  const candidateSearches = state.otp.goMode?.onboard?.candidateSearches || []
  if (candidateSearches.length === 0) return false
  return candidateSearches.every((cs) => {
    const search = state.otp.searches[cs.searchId]
    return !!search && !search.pending
  })
}
