...
import GoModeScreen from '../go-mode/GoModeScreen'
...
import ServerUnreachableBanner from './server-unreachable-banner'
...
/**
 * Whether a live Go Mode trip owns the map camera, and the planner's
 * auto-centering below must therefore keep its hands off it.
 *
 * There is exactly one map instance in this app: GoModeMap wraps the same
 * DefaultMap the planner uses (the `withMap`/`useMap().default` map), so
 * `setMapCenter`/`fitMapToPoints` in componentDidUpdate pan the map the rider
 * is looking at mid-trip. On a RESUMED trip that is not a one-off nudge but a
 * loop: the resume clears the planner's search (`CLEAR_ACTIVE_SEARCH`, so
 * `activeItinerary === null`), `getCurrentPosition` seeds `query.from` with
 * Current Location and there is no `query.to`, and this component re-renders on
 * every Go Mode tick because `goMode` is in its mapStateToProps — so
 * `setMapCenter(map, query.from)` fired several times a second and pinned the
 * camera on the rider's GPS point. The rider could not pan (2026-09-04,
 * 11:06:03: "I can no longer move around on the screen. It's locked to my gos
 * point"), and the Go Mode follow toggle looked broken because it was never
 * involved — `goMode.ui.mapFollowUser` was correctly false throughout.
 *
 * Backgrounded is the exception, as in `hidePlannerItineraryOverlay`: the
 * rider has stepped out to the planner, the planner's map is what is on
 * screen, and its auto-fly is wanted. Exported for unit tests.
 */
export function goModeOwnsMapCamera(goMode) {
  return !!goMode?.isActive && !goMode?.ui?.backgrounded
}

// Exported for unit tests: componentDidUpdate's camera gate is asserted
// directly on the prototype, without a store or a map.
export class ResponsiveWebapp extends Component {
  static propTypes = {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (currentPosition !== prevProps.currentPosition) {
      ...
      // if in mobile mode and from field is not set, use current location as
      // from and recenter map. (`== null` on purpose: the initial query has
      // `from: undefined`, and the strict null check left this auto-populate
      // dead — riders had to set Current Location by hand every session.)
      if (isMobile() && query.from == null) {
        setLocationToCurrent({ locationType: 'from' }, intl)
        // Seeding `from` is fine mid-trip; moving the camera is not.
        if (!goModeOwnsMapCamera(goMode)) setMapCenter(map, pt)
      }
    } else if (
      mainPanelContent === null &&
      autoFly !== false &&
      activeItinerary === null &&
      !goModeOwnsMapCamera(goMode)
    ) {
      if (
        query.from &&
        query.to &&
  ...
  componentDidMount() {
    ...
    if (isMobile()) {
      ...
      // refreshCurrentPosition, not getCurrentPosition: during a Go Mode trip
      // the native watcher is already streaming the rider's position at ~1 Hz,
      // and this poll was spending a second GPS acquisition to learn the same
      // thing — a median 31.0 s for entire sessions, including 202 of them
      // across the parked 104-minute 2026-08-31 mount. Outside a trip it still
      // asks the radio. The id is kept so the interval dies with the component
      // instead of outliving it.
      this._currentPositionIntervalId = setInterval(() => {
        this.props.refreshCurrentPosition(this.props.intl)
      }, 30000)
    }
  ...
  componentWillUnmount() {
    ...
    if (this._currentPositionIntervalId) {
      clearInterval(this._currentPositionIntervalId)
      this._currentPositionIntervalId = null
    }
  }
  ...
  renderDesktopView = () => {
    const { goMode, sessionTimeoutSeconds } = this.props
    const { MainControls, MainPanel, MapWindows } = this.context
    const { popupContent } = this.props

    // Backgrounded = trip still running but the rider stepped out to browse
    // the planner; fall through to the normal layout (with the
    // ReturnToTripBanner as the way back).
    if (goMode?.isActive && !goMode?.ui?.backgrounded) {
      return (
        <div className="otp">
          {/* Both layouts carry it: an unreachable server is worth saying
...
const mapDispatchToProps = {
  ...
  refreshCurrentPosition: locationActions.refreshCurrentPosition,
