...
class MobileMain extends Component {
  ...
  render() {
    ...
    // Check for viewed stop. A stop opened from inside a running trip is
    // marked inGoMode (actions/ui#viewStopFromItinerary) and is rendered by
    // the Go Mode layer over the live trip — swapping the whole screen for the
    // stop viewer there would tear the tracking screen down mid-ride.
    if (uiState.viewedStop && !uiState.viewedStop.inGoMode) {
