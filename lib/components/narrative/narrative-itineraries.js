...
import ConnectionFirstResults from './connection-first-results'
 ...
 * run that boards or alights elsewhere is kept off the links and stays under
 * the "N options" toggle, which names the other stop (backlog 28.4).
 */
...
/**
 * The connection-first regrouping (backlog 21.5) of the itineraries on screen,
 * rebuilt only when the list itself changes: the results component highlights
 * the chosen trip on the map, which is a store change, which re-runs
 * mapStateToProps — a fresh grouping each time would hand it new objects for
 * the same trips on every tick.
 */
let lastConnectionInput = null
let lastConnectionLock = null
let lastConnectionChoices = []
export function connectionChoicesFor(itineraries, routeLock) {
  if (itineraries !== lastConnectionInput || routeLock !== lastConnectionLock) {
    lastConnectionInput = itineraries
    lastConnectionLock = routeLock
    lastConnectionChoices = groupConnections(
      applyRouteLockToItineraries(itineraries || [], routeLock)
    )
  }
  return lastConnectionChoices
}
...
class NarrativeItineraries extends Component {
  ...
  /**
   * Screen 3 of the connection-first list: the app's own itinerary card,
   * opened (active + expanded, so its legs and Start button show) on the
   * chosen departure. Its departure chips choose among this connection's own
   * buses through `onChoose`. A leg tap is not wired: here it would switch the
   * whole panel into the itinerary view for an itinerary the store has not
   * been told is active.
   */
  _renderConnectionItinerary = (itinerary, onChoose) => {
    const {
      activeStep,
      realtimeEffects,
      setActiveStep,
  ...
  _showConnectionItinerary = (index) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {connectionFirst && !itineraryIsExpanded ? (
            // Backlog 21.5, behind Settings > "Choose stops first": where you
            // get on, then where you get off, and only then any time at all.
            <ConnectionFirstResults
              choices={connectionChoices}
              onShow={this._showConnectionItinerary}
              renderItinerary={this._renderConnectionItinerary}
            />
...
const mapStateToProps = (state) => {
  ...
  // Connection-first results (21.5) regroup every itinerary, not the merged
  // rows, because the route-signature merge folds different stops together.
  // Off unless the rider turned it on; then nothing below is computed.
  const connectionFirst = isConnectionFirstEnabled()
  const connectionChoices = connectionFirst
    ? connectionChoicesFor(allItineraries, routeLock)
    : null
