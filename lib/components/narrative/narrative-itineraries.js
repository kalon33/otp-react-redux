    ...
    const mergedItineraries = itineraries
      .reduce((prev, cur) => {
        ...
        if (duplicateIndex === -1 || cur.legs.some(isFlex)) {
          ...
          // A run the row's "Other stops" looked up (backlog 21.5) is an
          // alternative TO the row, never the row: a get-on stop further up
          // the line leaves earlier and would otherwise take the row over
          // under the rider's thumb, right after they tapped it.
          if (
            duplicateFoundItin.startTime > updatedItinerary.startTime &&
            !updatedItinerary.otherStopsLookup
          ) {
    ...
    return (
      ...
      >
        ...
        <ItineraryBody
          ...
          // This row is a result of the active search: its "Other stops" may
          // look the other stops up (backlog 21.5).
          otherStopsLookup
          role="listitem"
