...
import { itineraryUsesRoute } from '../../util/route-lock'
import {
  setActiveItinerary,
  setActiveLeg,
  setActiveStep,
...
class NarrativeItineraries extends Component {
  ...
  _renderItineraryRow = (itinerary, mini = false) => {
    ...
    const skipsLockedRoute =
      routeLock && !itineraryUsesRoute(itinerary, routeLock.id)
    ...
    return (
      ...
      >
        {skipsLockedRoute && (
          <S.OffRouteNote>
            <FormattedMessage
              id="components.BatchSearchScreen.routeLockNotUsed"
...
const mapStateToProps = (state) => {
  ...
  // With a route lock on, OTP still answers with the bike-the-whole-way option.
  // It's worth keeping — sometimes it really is the better trip — but it isn't
  // what the rider asked for, so the ones that actually ride the named route go
  // first. Stable partition: the sort above still decides the order within each
  // group.
  const { routeLock } = state.otp.currentQuery
  if (routeLock) {
    const usesRoute = (itin) => itineraryUsesRoute(itin, routeLock.id)
    mergedItineraries = [
      ...mergedItineraries.filter(usesRoute),
      ...mergedItineraries.filter((itin) => !usesRoute(itin))
    ]
  }
