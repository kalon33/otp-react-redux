...
const mapStateToProps = (state) => {
  ...
  const showPlanFirstLastButtons = state.otp.config.itinerary?.showPlanFirstLastButtons
  const fieldTripVisible = state.callTaker && state.callTaker.fieldTrip?.visible
  return {
    enabled: showPlanFirstLastButtons && fieldTripVisible === false,
    itineraries
  }
}
