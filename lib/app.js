...
import { Plane } from '@styled-icons/fa-solid/Plane'
import { Train } from '@styled-icons/fa-solid/Train'
...
import { AppErrorBoundary } from './components/util/error-boundary'
...
import GoModeDemo from './components/go-mode/GoModeDemo'
// Standalone Go Mode card gallery for visual review (no backend/sim/redux),
// reached at any URL with ?goModeDemo=1 — see GoModeDemo.tsx.
...
// Without a Bugsnag key this used to be React.Fragment — i.e. NO boundary at
// all. On 2026-09-04 a map-popup render exception therefore unmounted the whole
// app and left the rider on a white screen 50 seconds into a live trip. Fall
// back to our own boundary instead, which shows a reload prompt and reports the
// error to the debug-log sink.
...
const SvgIcon = ({ className, Fallback, iconName, style }) => {
  ...
  switch (iconName) {
    ...
    case 'bus':
      return <Bus className={className} style={style} />
    case 'plane':
      return <Plane className={className} style={style} />
    case 'train':
      return <Train className={className} style={style} />
    default:
      if (Fallback) {
...
const Webapp = () => {
  if (
    ...
    window.location.search.includes('goModeDemo')
  ) {
    return <GoModeDemo />
  }
  return (
